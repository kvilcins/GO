const createFooter = () => {
  ...
  mainWrap.className = 'contacts__main-wrap contacts-info';

  const innerWrap1 = document.createElement('div');
  innerWrap1.className = 'contacts-info__inner-wrap contacts-addresses';

  const address = document.createElement('address');
  address.className = 'contacts-addresses__info icons-wrap';

  const p1 = document.createElement('p');
  p1.className = 'icons-wrap__detail icons-wrap__detail_address';
  p1.innerHTML = '<span class="icons-wrap__detail_flex">г. Москва <span>ул. Усачёва, 29 к3</span></span>';

  const p2 = document.createElement('p');
  p2.className = 'icons-wrap__detail icons-wrap__detail_phone';

  const a1 = document.createElement('a');
  a1.href = 'tel:+79509302893';
  a1.className = 'icons-wrap__detail-link';
  a1.textContent = '8 (950) 930 - 28 - 93';

  const p3 = document.createElement('p');
  p3.className = 'icons-wrap__detail icons-wrap__detail_email';

  const a2 = document.createElement('a');
  a2.href = 'mailto:GameOver@gmail.com';
  a2.className = 'icons-wrap__detail-link';
  a2.textContent = 'GameOver@gmail.com';

  const ul = document.createElement('ul');
  ul.className = 'contacts-addresses__list';

  // Создание и настройка SVG-элементов
  const createSvgElement = (dValues, link) => {
    const li = document.createElement('li');
    li.className = 'contacts-addresses__item social-icon';

    const a = document.createElement('a');
    a.href = link;
    a.target = '_blank';
    a.className = 'social-icon__link link-wrap';
    ...
    svg.setAttribute('class', 'link-wrap__svg');

    dValues.forEach(dValue => {
  ...
  ul.appendChild(li3);
  ul.appendChild(li4);
  innerWrap1.appendChild(address);
  innerWrap1.appendChild(ul);
  innerWrap2.appendChild(iframe);
  p5.appendChild(a7);
  container2.appendChild(p4);
  container2.appendChild(p5);
  mainWrap.appendChild(innerWrap1);
  mainWrap.appendChild(innerWrap2);
  container1.appendChild(title);
  container1.appendChild(mainWrap);
  contactsSection.appendChild(container1);
  contactsSection.appendChild(container2);
  footer.appendChild(contactsSection);

  // Вставка элемента в конец body
